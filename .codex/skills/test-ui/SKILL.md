...
2. Run `powershell -ExecutionPolicy Bypass -File .codex/skills/test-ui/scripts/run-ui-tests.ps1` from the repository root. The script compiles the Java sources with Java 25 and runs `Device_G.boot()` through a temporary test launcher, bypassing the aesthetic introduction.
